...
{
    public partial class SmsCount : FrontBasic
    {
        ...
        private void GetChartJson()
        {
            ...
            var temp = SiteFund.GetProductFunds(TUser.UserId, 1);//1,3,4
            var account = temp.SyAccount;

            var result = api.QueryReportSys(typeof(SmsCount), account ?? "", startDate.ToString("yyyyMMdd"), endDate.ToString("yyyyMMdd"));
        ...
        private void GetPage()
        {
            ...
            {
                var result = api.QueryMtSys(typeof(SmsCount), account ?? "", phone, dt);
